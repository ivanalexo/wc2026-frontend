...
export default async function TeamsPage() {
  ...
  // Mapa nombre → probabilidad campeón
  ...
  // Ordenar por probabilidad de campeón descendente
  const sorted = [...teams].sort(
    (a, b) => (champMap[b.name] ?? 0) - (champMap[a.name] ?? 0)
  );

  // Agrupar por grupo para la sección secundaria
    ...
    <Box sx={{ py: { xs: 4, md: 6 } }}>
      <Container maxWidth="lg">
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2 }}>
          ...
          <Typography variant="h4" sx={{ fontWeight: 800 }}>Selecciones</Typography>
          <Typography variant="body2" color="text.secondary" sx={{ ml: 1 }}>
            {teams.length} equipos · ordenados por probabilidad de campeón
          </Typography>
        </Box>

        <Typography variant="body2" color="text.secondary" sx={{ mb: 4, opacity: 0.6 }}>
          Haz clic en un equipo para ver su perfil, probabilidades por ronda y partidos del grupo
        </Typography>

        {teams.length === 0 ? (
          <Box sx={{ textAlign: "center", py: 10 }}>
            <GroupsIcon sx={{ fontSize: 56, color: "rgba(255,255,255,0.1)", mb: 2 }} />
